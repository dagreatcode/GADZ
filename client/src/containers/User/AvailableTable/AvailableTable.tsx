...
const AvailableTable: React.FC = () => {
  ...
  const handle123Search = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || "Error fetching 123Loadboard search results");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={styles["at-container"]}>
      ...
      <section className={styles["at-section"]}>
        <h2>👨‍✈️ Drivers</h2>
        <div className={styles["at-buttonGroup"]}>
          <button onClick={fetchDrivers} className={styles["at-button"]}>Fetch Drivers</button>
        </div>
        <Table data={driverList} title="All Drivers" isUser={true} />
        <Table data={userDrivers} title="Your Drivers" isUser={true} showCompanyLink={false} />
      ...
      <section className={styles["at-section"]}>
        <h2>📦 Loads</h2>
        <div className={styles["at-buttonGroup"]}>
          <button onClick={fetchLoads} className={styles["at-button"]}>Fetch Loads</button>
        </div>
        <Table data={loads} title="All Loads" isUser={false} showCompanyLink />
      ...
      {/* 123Loadboard Search Section */}
      <section className={styles["at-section"]}>
        <h2>🔎 123Loadboard Search</h2>
        {!code && (
          <button onClick={handleAuthorizeNavigation} className={styles["at-button"]}>
            Authorize with 123Loadboard
          </button>
        )}

        {code && (
          <>
            <form className={styles["at-form"]} onSubmit={(e) => { e.preventDefault(); handle123Search(); }}>
              <input className={styles["at-input"]} type="text" name="originCity" placeholder="Origin City" value={searchFormData.originCity} onChange={handleInputChange} />
              <input className={styles["at-input"]} type="text" name="originState" placeholder="Origin State" value={searchFormData.originState} onChange={handleInputChange} />
              <button type="button" onClick={handleAutoFill} className={styles["at-button"]}>Auto Fill</button>
              <button type="submit" className={styles["at-button"]}>Search 123Loadboard</button>
            </form>

            {loading && <p>Loading...</p>}
            {error && <p className={styles["at-error"]}>{error}</p>}
            {success && <p className={styles["at-success"]}>Search successful!</p>}

            <div className={styles["at-scrollTable"]}>
              <Table data={searchResults} title="123Loadboard Results" isUser={false} />
            </div>
          </>
        )}
      </section>
