...
const AvailableTable: React.FC = () => {
  ...
  const location = useLocation();
  const queryParams = new URLSearchParams(location.search);
  const code = queryParams.get("code"); // Authorization code from OAuth
  ...
  const handle123Search = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.loads) {
        setSearchResults(response.data.loads.map(mapApiLoadToDisplay));
        setSuccess(true);
      } else {
        setError("No results found or failed search");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className={styles["at-container"]}>
      <header className={styles["at-hero"]}>
        ...
        <h1 className={styles["at-title"]}>🚛 GADZConnect Load Board</h1>
        <p className={styles["at-subtitle"]}>
          Manage drivers, loads, and connect with 123Loadboard seamlessly
        </p>
        <Link to="/User" className={styles["at-homeLink"]}>
          Back to Dashboard
        </Link>
      ...
      <section className={styles["at-section"]}>
        ...
        {searchResults.length > 0 && (
          <div className={styles["at-tableContainer"]}>
            <Table data={searchResults} title="Search Results" isUser={false} />
          </div>
        )}
      </section>
    </div>
