...
const AvailableTable: React.FC = () => {
  ...
  // Fetch 123Loadboard data manually only
  const fetchLoadboardData = useCallback(async (authCode: string) => {
    ...
    try {
      ...
      const mapped = (response.data.loads || []).map(mapApiLoadToDisplay);
  ...
  // NOTE: No auto-search after authorization
  useEffect(() => {}, [code]);

  // Fetch local loads
  const fetchLoads = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_SOCKET_IO_CLIENT_PORT}/api/loads`);
      setLoads(res.data);
      const userId = localStorage.getItem("userId");
      if (userId) setUserLoads(res.data.filter((l: Load) => l.userId === userId));
    } catch (err) {
      console.error("Error fetching loads:", err);
    }
  };
  ...
  const fetchDrivers = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_SOCKET_IO_CLIENT_PORT}/api/drivers`);
      setDriverList(res.data);
      const userId = localStorage.getItem("userId");
      if (userId) setUserDrivers(res.data.filter((d: Driver) => d.userId === userId));
    } catch (err) {
      console.error("Error fetching drivers:", err);
    }
  };
  ...
  // Handle form inputs
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setSearchFormData((prev) => ({ ...prev, [e.target.name]: e.target.value }));
  ...
  const handle123Search = async () => {
    if (!code) {
      setError("Authorization code missing. Please authorize first.");
      return;
    }
    setLoading(true);
    setError(null);
  ...
  return (
    <div className={styles["at-container"]}>
      ...
      <section className={styles["at-section"]}>
        <h2>👨‍✈️ Drivers</h2>
        <div className="mb-3">
          <button onClick={fetchDrivers} className="btn btn-primary at-button">
            Fetch Drivers
          </button>
        </div>
        <Table data={driverList} title="All Drivers" isUser={true} />
      ...
      <section className={styles["at-section"]}>
        <h2>📦 Loads</h2>
        <div className="mb-3">
          <button onClick={fetchLoads} className="btn btn-primary at-button">
            Fetch Loads
          </button>
        </div>
        <Table data={loads} title="All Loads" isUser={false} showCompanyLink />
        <Table data={userLoads} title="Your Loads" isUser={false} showCompanyLink={false} />
      </section>

      {/* 123Loadboard */}
      <section className={styles["at-section"]} ref={tableRef}>
        <h2>🌐 123Loadboard</h2>
        <div className="mb-3 d-flex gap-2">
          <button onClick={handleAuthorizeNavigation} className="btn btn-outline-primary at-buttonAlt">
            🔑 Authorize
          </button>
          <button onClick={() => fetchLoadboardData(code || "")} className="btn btn-primary at-button">
            📥 Fetch 123Loadboard Data
          </button>
        </div>
        ...
        <form className={styles["at-form"]} onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="row g-2">
            {Object.keys(searchFormData).map((key) => (
              <div key={key} className="col-md-3">
                <input
                  className="form-control at-input"
                  type={key.includes("Date") ? "date" : "text"}
                  name={key}
                  placeholder={key}
                  value={searchFormData[key as keyof typeof searchFormData]}
          ...
          <div className="text-center mt-3">
            <button type="button" className="btn btn-secondary me-2" onClick={handleAutoFill}>
              Autofill
            </button>
            <button type="button" className="btn btn-success" onClick={handle123Search}>
              {loading ? "Searching..." : "Search 123Loadboard"}
            </button>
          </div>

          {error && <p className="text-danger text-center mt-2">{error}</p>}
          {success && <p className="text-success text-center mt-2">Search completed!</p>}
        </form>
        ...
        {searchResults.length > 0 && (
          <div className={styles["at-scrollableTableContainer"]}>
            <div className={styles["at-scrollableTableWrapper"]}>
              <table className="table table-striped at-table">
                ...
                <tbody>
                  {searchResults.map((load) => (
                    <tr key={load.id}>
                      ...
                      <td>{load.computedMileage}</td>
                      <td>{load.numberOfLoads}</td>
                      <td>{load.pickupDateTimesUtc?.join(", ") || "N/A"}</td>
                      <td>{load.equipments?.map((e) => e.name || e.code || e).join(", ")}</td>
                      <td>{load.privateLoadNote}</td>
                      <td>{load.status}</td>
                      <td>
                        {load.originLocation
