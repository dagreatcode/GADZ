...
router.get("/pay", async (req, res) => {
  try {
    const allLetters = await db.NewsLetter.findAll();
    res.json(allLetters);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Error fetching newsletters." });
...
router.post("/pay", async (req, res) => {
  try {
    const newLetter = await db.NewsLetter.create(req.body);
...
router.put("/:id", async (req, res) => {
  try {
    const { subject, description, archived, important } = req.body;

    const updatedNewsletter = await db.NewsLetter.update(
      { subject, description, archived, important },
      { where: { id: req.params.id }, returning: true }
    );

    // Sequelize returns [numberOfAffectedRows, affectedRowsArray]
    res.json(updatedNewsletter[1][0]);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Error updating newsletter." });
  }
